...
1) Прошивку микроконтроллера в папке bin
...
4. Отключите программатор от платы и компьютера и подключитесь к компьютеру с помощью USB кабеля через разъем Micro-USB на плате
5. Посмотрите в диспетчере устройств во вкладке "Порты" номер COM port подключенной платы
6. Для Windows установите Termite 3.4 по ссылке https://termite.software.informer.com/download/#downloading
Для ОС на Linux выполните команду sudo apt-get install putty, она установит аналогичную программу для работы с консолью через последовательный порт
7. Запустите программу и настройте в ней нужный номер COM порта, а также скорость передачи 230400 бод
